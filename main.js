...
function showColorPicker(x, y, clickedObject) {
    ...
    colors.forEach((color, index) => {
        ...
        // On click, change the clicked object's color
        colorButton.addEventListener('click', () => {

            clickedObject.material.color.set(color);
            document.body.removeChild(colorPickerContainer); // Remove the color picker after selection
        });
    });

    // Close the color picker if clicked anywhere outside
...
window.addEventListener('mouseup', function(event) {
    ...
    if(isPlacing) {
        ...
        for (const hazard of hazardObjects) {
            ...
            if (distance >2.0) { // Threshold for hazard proximity
...
window.addEventListener("contextmenu", function (event) {
    ...
    if (intersects.length > 0) {
      const intersect = intersects[0];
      if (intersect.object.name.startsWith("12283_Chair_v1")) {
        const popUp = document.createElement("div");
        popUp.className = "popup";
